  ...
  layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element','colorpicker', 'slider'], function(){
   ...
   //执行一个 table 实例
    table.render({
      elem: '#dmlist'
      ,height: 500
      ,url: '/dmku/index.php?ac=list' //数据接口
      ,title: '用户表'
      ,page: true //开启分页
    ...
    table.render({
      elem: '#dmreport'
      ,height: 480
      ,url: '/dmku/index.php?ac=reportlist' //数据接口
      ,title: '用户表'
      ,page: true //开启分页
      //,toolbar: 'default' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
      //,totalRow: true //开启合计行
   ...
   form.on('submit(bond_sumbit)', function (data) {
       ...
       form.on('submit(reloadlst_submit)', function (data) {
           ...
           table.render({
              elem: '#dmlist'
              ,height: 500
              ,url: '/dmku/index.php?ac=so&key='+key //数据接口
              ,title: '用户表'
              //,page: true //开启分页
              //,toolbar: 'default' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
              //,totalRow: true //开启合计行
      ...
      function delreport (id) {$.ajax({url: "/dmku/index.php?ac=del&type=report&id="+id,success: function(data){}});}
      function delid (id) {$.ajax({url: "/dmku/index.php?ac=del&type=list&id="+id,success: function(data){}});}
   });
